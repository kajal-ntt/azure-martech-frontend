...
export interface TextStyle {
  ...
  borderRadius?: number;             // rx/ry for rect shapes (0–200)
  ...
  // Highlight — rendered as a full-textbox background rect (not per-line)
  ...
  objWidth?: number;
  objHeight?: number;
  // Curved text — when the selected object is a curved-text image
...
export interface EditorCanvasRef {
  ...
  addElement: (element: import("./components/editor/Toolbar").ElementType) => void;
...
// Crop box state in % of canvas display size (0–100)
interface CropBox { x: number; y: number; w: number; h: number; }

function getTextStyleFromObject(obj: fabric.IText): TextStyle {
  const getShadow = (s: any) => {
    if (!s) return { color: "#000000", blur: 0, x: 0, y: 0 };
    return {
  ...
  const base: TextStyle = {
    ...
    // Highlight — stored as custom properties on the text object
  ...
  // If in editing mode, try to get styles from the current selection
  if (obj.isEditing) {
    const selectionStyles = obj.getSelectionStyles();
    if (selectionStyles && selectionStyles.length > 0) {
      const s = selectionStyles[0] as any;
      if (s.fontFamily !== undefined) base.fontFamily = s.fontFamily;
      if (s.fontSize !== undefined) base.fontSize = s.fontSize;
      if (s.fill !== undefined) base.fill = s.fill;
      if (s.fontWeight !== undefined) base.fontWeight = s.fontWeight;
      if (s.fontStyle !== undefined) base.fontStyle = s.fontStyle;
      if (s.underline !== undefined) base.underline = s.underline;
      if (s.linethrough !== undefined) base.linethrough = s.linethrough;

      if (s.shadow) {
        const ssh = getShadow(s.shadow);
        base.shadowColor = ssh.color;
        base.shadowBlur = ssh.blur;
        base.shadowOffsetX = ssh.x;
        base.shadowOffsetY = ssh.y;
      }
...
const EditorCanvas = forwardRef<EditorCanvasRef, EditorCanvasProps>(
  function EditorCanvas({ layers, onDirty, onSelectionChange, onLayersChange, onImageLoaded }, ref) {
    const canvasElRef = useRef<HTMLCanvasElement>(null);
    const wrapperRef = useRef<HTMLDivElement>(null);
    const fabricRef = useRef<fabric.Canvas | null>(null);
    const naturalSizeRef = useRef({ w: CANVAS_WIDTH, h: CANVAS_HEIGHT });
    ...
    // Crop state — pure React, no fabric involvement
    ...
    const emitLayers = (canvas: fabric.Canvas) => {
      ...
      onLayersChangeRef.current(infos.reverse());
    };

    // ── Highlight rect management ─────────────────────────────────────────────
    ...
    useEffect(() => {
      ...
      const handleWheel = (e: WheelEvent) => {
        // Always zoom when over the canvas — prevent page scroll
        e.preventDefault();
      ...
      const handleSelection = () => {
        ...
        if (active && (active.type === "i-text" || active.type === "textbox")) {
          onSelectionChangeRef.current(getTextStyleFromObject(active as fabric.IText));
        } else if (active) {
          ...
          onSelectionChangeRef.current({
            ...
            strokeWidth: (active.strokeWidth as number) || 0,
      ...
      const tryLoad = () => {
        ...
        const loadLayer = async (layer: LayerDef) => {
          ...
          if (layer.type === "shape" && layer.id === "bottom-band") {
            ...
            const bandH = Math.round(CANVAS_HEIGHT * heightPct);
            const band = new fabric.Rect({
              left: 0,
              top: CANVAS_HEIGHT - bandH,
              width: CANVAS_WIDTH,
              height: bandH,
              fill: bandColor,
              selectable: true,
            ...
            (band as any).__layerId = layer.id;
            (band as any).__layerLabel = layer.label;
            (band as any).__layerType = "shape";
            canvas.add(band);
            // Stack: above background, below text/logo
            canvas.sendObjectToBack(band);
            const bgObj = canvas.getObjects().find(o => (o as any).__layerType === "background");
            if (bgObj) {
              canvas.moveObjectTo(band, canvas.getObjects().indexOf(bgObj) + 1);
            }
            canvas.renderAll();
            return;
          }
          ...
          if (layer.type === "text") {
            ...
            const topPos = layer.top ?? (isHeadline ? CANVAS_HEIGHT * 0.25 : CANVAS_HEIGHT - 120);
            ...
            (text as any).__layerId = layer.id;
            (text as any).__layerLabel = layer.label;
          ...
          try {
            ...
            if (layer.type === "background") {
              ...
              (img as any).__layerLabel = layer.label;
              (img as any).__layerType = "background";
              canvas.add(img);
              ...
              const positions: Record<string, { left: number; top: number }> = {
                logo: { left: CANVAS_WIDTH - 80, top: 80 },
                graphic: { left: CANVAS_WIDTH / 2, top: CANVAS_HEIGHT / 2 },
                actors: { left: CANVAS_WIDTH / 2, top: CANVAS_HEIGHT * 0.6 },
              };
              const pos = positions[layer.type] ?? { left: CANVAS_WIDTH / 2, top: CANVAS_HEIGHT / 2 };
              img.set({
                scaleX: scale, scaleY: scale, left: pos.left, top: pos.top,
                originX: "center", originY: "center",
                selectable: true, evented: true, hasControls: true, hasBorders: true,
              });
              (img as any).__layerId = layer.id;
              (img as any).__layerLabel = layer.label;
              (img as any).__layerType = layer.type;
              canvas.add(img);
            }
            canvas.renderAll();
            onImageLoaded?.();
          } catch {
            loadedLayerIdsRef.current.delete(layer.id);
          }
        };
      ...
      const onMouseMove = (e: MouseEvent) => {
        ...
        setCropBox(prev => {
          ...
          if (drag.type === "move") {
            ...
            if (drag.type.includes("e")) { w = Math.max(MIN, Math.min(100 - b.x, b.w + dx)); }
            if (drag.type.includes("w")) { const nw = Math.max(MIN, b.w - dx); x = b.x + b.w - nw; w = nw; }
            if (drag.type.includes("s")) { h = Math.max(MIN, Math.min(100 - b.y, b.h + dy)); }
            if (drag.type.includes("n")) { const nh = Math.max(MIN, b.h - dy); y = b.y + b.h - nh; h = nh; }
            // Clamp
            if (x < 0) { w += x; x = 0; }
            if (y < 0) { h += y; y = 0; }
            if (x + w > 100) w = 100 - x;
    ...
    useImperativeHandle(ref, () => ({
      addText() {
        ...
        const text = new fabric.Textbox("Double-click to edit", {
          left: CANVAS_WIDTH / 2, top: CANVAS_HEIGHT / 2,
          width: CANVAS_WIDTH * 0.7, originX: "center", originY: "center",
          fontFamily: "Arial", fontSize: 28, fill: "#ffffff", textAlign: "center",
        });
      ...
      addImageLayer(url: string, label: string, id: string) {
        ...
        fabric.FabricImage.fromURL(url, { crossOrigin: "anonymous" }).then(img => {
          ...
          img.set({ scaleX: scale, scaleY: scale, left: CANVAS_WIDTH / 2, top: CANVAS_HEIGHT / 2, originX: "center", originY: "center" });
      ...
      getSelectedTextStyle() {
        ...
        const active = canvas.getActiveObject();
        if (active?.type === "i-text") return getTextStyleFromObject(active as fabric.IText);
        return null;
      },
      ...
      applyTextStyle(style: Partial<TextStyle>) {
        ...
        if (active) {
          ...
          // Handle highlight — stored as custom props, rendered as a sibling Rect
          const isTextObj = active.type === "i-text" || active.type === "textbox";
          if (isTextObj && (
          ...
          // Handle shadow separately as it requires a fabric.Shadow instance
          if (
            ...
            const current = (isText && (active as any).isEditing ? ((active as any).getSelectionStyles()[0] as any)?.shadow : active.shadow) as any;
          ...
          // Ensure uniform stroke for non-text objects
          if (active.type !== "i-text" && active.type !== "textbox" && updates.strokeWidth !== undefined) {
            updates.strokeUniform = true;
          }

          // Handle gradient fill for non-text objects
          if (
            ...
            const isTextObj2 = active.type === "i-text" || active.type === "textbox";
            if (!isTextObj2) {
              // Store gradient params as custom props
              ...
              if (enabled) {
                // Fabric v7: gradient coords are in object's own (unscaled) coordinate space
                const w = active.width  ?? 100;
                const h = active.height ?? 100;
                const rad = (angle * Math.PI) / 180;
                if (gType === "radial") {
                  // focalR controls where the inner stop sits: 0 = tight dot at center, 1 = full radius
                  const outerR = Math.max(w, h) / 2;
                  const innerR = outerR * focalR;
                  const grad = new fabric.Gradient({
                    type: "radial",
                    gradientUnits: "pixels",
                    coords: { x1: w / 2, y1: h / 2, r1: innerR, x2: w / 2, y2: h / 2, r2: outerR },
          ...
          // Handle strokeDashArray (border style)
          ...
          if ((active.type === "i-text" || active.type === "textbox") && (active as any).isEditing) {
            // Apply only to the current text selection
          ...
          // Re-emit style to keep panel in sync
          if (active.type === "i-text" || active.type === "textbox") {
            onSelectionChangeRef.current(getTextStyleFromObject(active as fabric.IText));
          } else {
            const isCurved2 = !!(active as any).__curvedTextSource;
            onSelectionChangeRef.current({
              fontFamily: "",
              fontSize: 0,
              fill: (active.fill as string) || "",
      ...
      getLayers() {
        const canvas = fabricRef.current;
        if (!canvas) return [];
        return canvas.getObjects().map(buildLayerInfo).filter(Boolean).reverse() as LayerInfo[];
      },
      ...
      exportBlob(): Promise<Blob> {
        return new Promise((resolve, reject) => {
          const canvas = fabricRef.current;
          if (!canvas) { reject(new Error("Canvas not initialized")); return; }
      ...
      addElement(element: import("./components/editor/Toolbar").ElementType) {
        const canvas = fabricRef.current;
        if (!canvas) return;
        ...
        const emojiMap: Record<string, string> = {
          heart: "❤️", check: "✅", cross: "❌",
          "badge-new": "🆕", "badge-sale": "🏷️", "badge-hot": "🔥",
      ...
      addBadge(badgeType: BadgeType) {
        ...
        const BADGE_CONFIGS: Record<BadgeType, { text: string; bg: string; textColor: string; shape: "circle" | "pill" | "burst" }> = {
          ...
          "badge-hot":     { text: "🔥 HOT",   bg: "#FF6F00", textColor: "#ffffff", shape: "pill" },
    ...
    return (
      <div ref={wrapperRef} className="relative flex items-center justify-center w-full h-full">
        ...
        {/* React crop overlay — sits exactly on top of the canvas element */}
        {cropMode && (
